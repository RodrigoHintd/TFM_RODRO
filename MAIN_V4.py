...
def create_data_model():
    ...
    data["demands"] = [
    0,      # depot
    -1,-1,-2,-4,-2,-4,-1,-1,-1,-2,   # entregas (negativas)
    3,2,4,1,3,2      # recogidas (positivas)
]
...
def print_solution(data, manager, routing, solution):
    ...
    for vehicle_id in range(data["num_vehicles"]):
        ...
        while not routing.IsEnd(index):
            ...
            # Obtenemos el valor REAL que el motor calculó para este nodo
        ...
        # Al llegar de vuelta al depósito
        time_var = solution.Value(time_dimension.CumulVar(index))
        load_var = solution.Value(capacity_dimension.CumulVar(index))
        ...
        # Solo imprimimos rutas que se mueven (distancia > 0)
        if route_distance > 0:
            print(plan_output)
...
def main():
    ...
    # 2. Añadir la dimensión con techo de 15 (el True es clave)
    routing.AddDimensionWithVehicleCapacity(
        demand_callback_index,
        0,  # null capacity slack
        data["vehicle_capacities"],  # capacidad máxima (15)
        False,  # start_cumulative_to_zero -> Ponerlo en True para que respete el SetValue
        "Capacity"
    )
    
    capacity_dimension = routing.GetDimensionOrDie("Capacity")

    # 3. Forzar a cada vehículo a empezar LLENO (15)
    for vehicle_id in range(data["num_vehicles"]):
    ...
    # REGLA DE ORO: Si ya estás en una recogida, prohibido ir a una entrega.
    for p in data["pickup_nodes"]:
        p_index = manager.NodeToIndex(p)
        for d in data["delivery_nodes"]:
            d_index = manager.NodeToIndex(d)
            # Esto elimina la posibilidad de que después de un nodo P vaya un nodo D
